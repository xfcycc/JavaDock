...
function parsePsPortsString(portsStr: string): DockerPort[] {
  if (!portsStr) return [];
...
function parsePsOutput(raw: Record<string, string>): DockerContainerSummary {
  return {
    id: raw.ID,
    ...
    image: raw.Image,
    status: raw.Status,
    state: (raw.State || 'created') as DockerContainerSummary['state'],
...
function parseInspectOutput(raw: Record<string, any>): DockerContainerDetail {
  ...
  // 解析挂载点
  const mounts: DockerMount[] = (raw.Mounts || []).map((m: any) => ({
    type: m.Type,
    source: m.Source,
...
/** 获取所有容器列表（包含已停止的） */
export async function listContainers(): Promise<DockerContainerSummary[]> {
...
/** 获取单个容器完整详情 */
export async function inspectContainer(id: string): Promise<DockerContainerDetail> {
...
export async function recreateContainer(
  id: string,
  overrides: DockerUpdateParams
): Promise<void> {
  const detail = await inspectContainer(id);

  // 构建 docker run 参数列表
  const args: string[] = ['run', '-d', `--name`, detail.name];
  ...
  // 停止并删除旧容器
  try { await execAsync(`docker stop ${id}`); } catch { /* 容器可能已停止 */ }
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`docker run 失败: ${stderr}`));
    });
  });
}
